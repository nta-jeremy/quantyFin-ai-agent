// @ts-nocheck
/* QuantyFin · Mock data
   - Real VN tickers + placeholder VN-tone news
   - 4 scenarios: up / down / volatile / crisis
   - Exposed via window.QF_DATA
*/

(function () {

// ── Vietnamese stock tickers (subset, realistic groupings) ─────────────
const STOCKS = [
  // Ngân hàng
...
// ── Base prices (realistic, in nghìn đồng) ─────────────────────────────
const BASE_PRICE = {
...
function generateSpark(ticker, scenario, n = 32) {
...
// ── Build scenario-dependent stock data ─────────────────────────────
function buildStocks(scenario) {
  return STOCKS.map((s) => {
    ...
    const dayChangePct = pctChange(series.slice(-2));
    // 5-day change
    ...
    return {
      ...
      confidence: ['high', 'med', 'low'][Math.floor(r * 3) % 3],
...
// ── Indices ─────────────────────────────────────────────────────────
function buildIndices(scenario) {
  function ix(name, baseVal, seed) {
...
// ── Macro events (always shown, scenario-tinted) ───────────────────
const MACRO_BANK_UP = [
...
// ── News pool (placeholders in real VN-news tone) ──────────────────
const NEWS_TEMPLATES = [
  // pos
  { tone: 'pos', src: 'CafeF',       fmt: (t) => `${t.ticker} báo lãi quý I/2026 tăng ${randPct(15,42)}%, vượt kế hoạch năm`, },
  { tone: 'pos', src: 'Vietstock',   fmt: (t) => `${t.ticker}: HĐQT thông qua chia cổ tức tiền mặt ${randPct(10,25)}%, lịch chốt 28/06`, },
  { tone: 'pos', src: 'NDH',         fmt: (t) => `Khối ngoại mua ròng ${randPct(80,420)} tỷ đồng cổ phiếu ${t.ticker} trong tuần`, },
  { tone: 'pos', src: 'VnEconomy',   fmt: (t) => `${t.ticker} ký thỏa thuận hợp tác chiến lược với đối tác Hàn Quốc, dự kiến doanh thu tăng ${randPct(8,18)}%`, },
  { tone: 'pos', src: 'ĐTCK',        fmt: (t) => `Báo cáo SSI: nâng khuyến nghị ${t.ticker} lên MUA, giá mục tiêu ${randInt(20,200)} đồng`, },
  // neg
  { tone: 'neg', src: 'CafeF',       fmt: (t) => `${t.ticker} báo lỗ quý I/2026, biên lợi nhuận gộp giảm ${randPct(3,9)} điểm phần trăm`, },
  { tone: 'neg', src: 'Vietstock',   fmt: (t) => `${t.ticker}: cổ đông lớn đăng ký bán ${randPct(2,8)}% vốn, cổ phiếu chịu áp lực`, },
  { tone: 'neg', src: 'Tuổi Trẻ',    fmt: (t) => `Khối ngoại bán ròng ${randPct(120,580)} tỷ đồng cổ phiếu ${t.ticker} trong tuần`, },
  { tone: 'neg', src: 'NDH',         fmt: (t) => `${t.ticker} bị UBCKNN xử phạt ${randPct(60,400)} triệu vì công bố thông tin không đầy đủ`, },
  // neutral
  { tone: 'neu', src: 'Vietstock',   fmt: (t) => `${t.ticker} công bố tài liệu ĐHCĐ thường niên 2026, kế hoạch lợi nhuận đi ngang`, },
  { tone: 'neu', src: 'CafeF',       fmt: (t) => `${t.ticker}: thay đổi nhân sự cấp cao, bổ nhiệm Phó Tổng Giám đốc mới`, },
  { tone: 'neu', src: 'VnBusiness',  fmt: (t) => `${t.ticker} hoàn tất phát hành ${randPct(200,800)} tỷ trái phiếu doanh nghiệp, kỳ hạn 3 năm`, },
];

function randPct(a, b) { return Math.round(a + Math.random() * (b - a)); }
function randInt(a, b) { return Math.round((a + Math.random() * (b - a)) * 1000); }

// Build news feed for scenario (fixed per scenario for reproducibility)
function buildNews(scenario, stocks) {
  ...
  for (let i = 0; i < 32; i++) {
    ...
    const confChoices = ['high','high','med','med','low'];
    ...
    const filterStatus = i % 11 === 5 ? 'filtered' : (i % 17 === 3 ? 'pending' : 'analyzed');
    items.push({
      id: 'n' + i,
      title: tpl.fmt(stock),
      src: tpl.src,
...
// ── Alerts ─────────────────────────────────────────────────────────
function buildAlerts(scenario, news, stocks) {
  const alerts = [];
  if (scenario === 'crisis') {
...
// ── Watchlist (subset) ─────────────────────────────────────────────
const WATCHLIST = ['VCB','FPT','HPG','MWG','VNM','VHM','SSI','VIC'];

// ── Knowledge graph (small but rich subgraph) ──────────────────────
const KG_NODES = [
  // macro events
...
// ── Crawler / pipeline jobs ────────────────────────────────────────
function buildJobs(scenario) {
