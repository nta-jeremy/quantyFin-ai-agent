...
export function Icon({ k, size = 16, stroke = 1.75, style }: IconProps) {
  const d = ICONS[k];
  if (!d) return null;
  return (
...
export function TPill({ children, tone, onClick, style }: TPillProps) {
  return (
    <span className={'t-pill ' + (tone || '')} style={style} onClick={onClick} role={onClick ? 'button' : undefined}>
      {children}
    </span>
  );
}
...
export function Sparkline({ data, tone = 'auto', width = 96, height = 32, withDot = true }: SparklineProps) {
...
export function Section({ title, meta, actions, flush, children, ai, style }: SectionProps) {
  return (
...
// ════════ FORMAT ════════
export function fmt(n: number | null | undefined, d = 2) { return n == null ? '—' : Number(n).toLocaleString('vi-VN', { minimumFractionDigits: d, maximumFractionDigits: d }); }
export function fmtInt(n: number | null | undefined) { return n == null ? '—' : Number(n).toLocaleString('vi-VN'); }
export function fmtPct(n: number | null | undefined, d = 2) { return n == null ? '—' : (n >= 0 ? '+' : '') + Number(n).toFixed(d) + '%'; }
export function fmtKbig(n: number) {
