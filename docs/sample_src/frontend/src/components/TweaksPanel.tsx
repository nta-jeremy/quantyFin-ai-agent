...
export function useTweaks<T extends Record<string, any>>(defaults: T): [T, (key: keyof T | Partial<T>, val?: any) => void] {
  const [values, setValues] = useState<T>(defaults);
  
  const setTweak = useCallback((keyOrEdits: keyof T | Partial<T>, val?: any) => {
    const edits = typeof keyOrEdits === 'object' && keyOrEdits !== null
      ? keyOrEdits : { [keyOrEdits as string]: val } as any;
...
export function TweaksPanel({ title = 'Tweaks', children }: TweaksPanelProps) {
  ...
  useEffect(() => {
    ...
    // Standalone panel is always open in preview mode, let's keep it visible
    setOpen(true); 
...
interface TwkCheckProps {
  light: boolean;
}

const __TwkCheck = ({ light }: TwkCheckProps) => (
  <svg viewBox="0 0 14 14" aria-hidden="true" style={{ width: 12, height: 12, fill: 'none' }}>
    <path d="M3 7.2 5.8 10 11 4.2" strokeWidth="2.2"
