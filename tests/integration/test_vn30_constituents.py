...
async def test_vn30_constituents_workflow():
    ...
    async with httpx.AsyncClient(
        ...
        assert (
            len(data) == 30
        ), f"Expected 30 VN30 constituents, got {len(data)}"
        ...
        assert set(data) == set(
            group_data
        ), "VN30 data mismatch between endpoints"
...
async def test_vn30_exchange_distribution():
    ...
    assert (
        len(vn30_exchange_info) == 30
    ), "Could not find all VN30 constituents in exchange data"
    ...
    assert (
        hose_count >= 25
    ), f"Expected at least 25 HOSE stocks in VN30, got {hose_count}"

    # Other exchanges should have minimal representation
    other_exchanges = sum(
        count for ex, count in exchanges.items() if ex != "HOSE"
    )
    assert (
        other_exchanges <= 5
    ), f"Too many non-HOSE stocks in VN30: {other_exchanges}"
...
async def test_vn30_company_names():
    ...
    async with httpx.AsyncClient(base_url=BASE_URL) as client:
        symbols_response = await client.get(
            "/listing/symbols", headers=headers
        )
        vn30_response = await client.get(
            "/listing/symbols/vn30", headers=headers
        )
    ...
    company_map = {
        symbol["ticker"]: symbol["organ_name"] for symbol in symbols_data
    }
    ...
    assert (
        not missing_names
    ), f"VN30 tickers missing company names: {missing_names}"
    assert (
        not invalid_names
    ), f"VN30 tickers with invalid company names: {invalid_names}"
    ...
    for ticker, expected_company in well_known_companies.items():
        if ticker in vn30_tickers:
            ...
            assert (
                ...
                or ticker.lower() in actual_company.lower()
...
async def test_vn30_error_handling():
    ...
    async with httpx.AsyncClient(base_url=BASE_URL) as client:
        response = await client.get(
            "/listing/symbols/vn30", headers=invalid_headers
        )
    ...
    async with httpx.AsyncClient(base_url=BASE_URL) as client:
        response = await client.get(
            "/listing/symbols/vn30?invalid=value", headers=headers
        )
...
async def test_vn30_performance():
    ...
    ), f"Data processing time {processing_time:.4f}s too slow"
