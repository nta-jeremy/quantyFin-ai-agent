...
class MigrationRunner:
    ...
    def run_migrations(
        self, migrations_dir: str, target_version: Optional[str] = None
    ) -> bool:
        ...
        try:
            ...
            for migration_file in migration_files:
                ...
                if version in applied_migrations:
                    logger.info(
                        f"Migration {version} already applied, skipping"
                    )
                    continue

                if target_version and version > target_version:
                    logger.info(
                        f"Reached target version {target_version}, stopping"
                    )
                    break
                ...
                if self.apply_migration(migration_file):
                    ...
                    logger.error(
                        f"Failed to apply migration {version}, stopping"
                    )
                    break

            logger.info(
                f"Migration completed: {success_count}/{total_pending} migrations applied"
            )
    ...
    def status(self, migrations_dir: str) -> None:
        ...
        try:
            ...
            for migration_file in migration_files:
                version = migration_file.stem
                status = (
                    "APPLIED" if version in applied_migrations else "PENDING"
                )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="QuantyFinAI Database Migration Runner"
    )
    parser.add_argument(
        "command",
    ...
    parser.add_argument(
        "--version", help="Version to rollback (for rollback command)"
    )
    ...
    if args.command == "migrate":
        success = runner.run_migrations(
            args.migrations_dir, args.target_version
        )
