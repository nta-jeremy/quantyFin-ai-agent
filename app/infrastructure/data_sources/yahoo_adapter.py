...
This module provides the infrastructure layer implementation for Yahoo Finance data source
...
class YahooAdapter:
    """Yahoo Finance data source adapter implementation."""

    def __init__(self, config: DataSourceConfig):
        """Initialize Yahoo adapter with configuration."""
        self.config = config
        self.session: Optional[aiohttp.ClientSession] = None
        self.metrics = DataSourceMetrics(data_source=DataSource.YAHOO)
    ...
    async def check_health(self) -> DataSourceHealth:
        """Check Yahoo Finance data source health."""
        ...
        try:
            ...
            params = {"symbol": "AAPL", "interval": "1D", "limit": 1}
    ...
    async def _respect_rate_limit(self) -> None:
        """Respect rate limiting for Yahoo Finance API."""
    ...
    async def get_historical_data(
        ...
        if not self.session:
            raise DataSourceUnavailableError(
                "Session not initialized", DataSource.YAHOO
            )
        ...
        if request.data_source and request.data_source.lower() == "yahoo":
            params["source"] = "yahoo"
        ...
        try:
            async with self.session.get(url, params=params) as response:
                ...
                if response.status == 200:
                    ...
                    raise SymbolNotFoundError(
                        f"Symbol {request.symbol} not found in Yahoo Finance",
                        request.symbol,
                        request.asset_type,
                    )
                elif response.status == 429:
                    ...
                    raise DataSourceUnavailableError(
                        f"Yahoo Finance server error: {response.status}",
                        request.symbol,
                        request.asset_type,
                    )
                else:
            ...
            raise TimeoutError(
                "Request to Yahoo Finance timed out",
                request.symbol,
                request.asset_type,
            )
            ...
            raise DataValidationError(
                f"Failed to parse Yahoo Finance response: {str(e)}",
                request.symbol,
                request.asset_type,
            )
            ...
            raise HistoricalDataError(
                f"Unexpected error with Yahoo Finance: {str(e)}",
                request.symbol,
                request.asset_type,
            )
        ...
        try:
            async with self.session.get(url, params=params) as response:
                ...
                if response.status == 200:
                    ...
                    raise DataSourceUnavailableError(
                        f"Yahoo Finance error: {response.status}",
                        symbol,
                        asset_type,
                    )
            ...
            raise NetworkError(
                f"Network error getting real-time quote from Yahoo Finance: {str(e)}",
                symbol,
                asset_type,
            )
            ...
            raise DataValidationError(
                f"Failed to parse Yahoo Finance quote response: {str(e)}",
                symbol,
                asset_type,
            )
            ...
            raise HistoricalDataError(
                f"Unexpected error with Yahoo Finance real-time quote: {str(e)}",
                symbol,
                asset_type,
            )
    ...
    async def get_intraday_data(
        ...
        try:
            ...
            raise NetworkError(
                f"Network error getting intraday data from Yahoo Finance: {str(e)}",
                symbol,
                asset_type,
            )
            ...
            raise DataValidationError(
                f"Failed to parse Yahoo Finance intraday response: {str(e)}",
                symbol,
                asset_type,
            )
            ...
            raise HistoricalDataError(
                f"Unexpected error with Yahoo Finance intraday data: {str(e)}",
                symbol,
                asset_type,
            )
    ...
    def _parse_historical_data(
        ...
        """Parse historical data response from Yahoo Finance."""
        data_points = []
        ...
        try:
            ...
            for record in records:
                try:
                    # Parse timestamp (Yahoo Finance might use different formats)
                    if "time" in record:
            ...
            raise DataValidationError(
                f"Failed to parse Yahoo Finance historical data: {str(e)}"
            )
    ...
    def _parse_real_time_quote(self, data: Dict[str, Any]) -> OHLCVTData:
        """Parse real-time quote response from Yahoo Finance."""
        try:
            ...
            volume = int(
                data.get(
                    "volume",
                    data.get("Volume", data.get("regularMarketVolume", 0)),
                )
            )
            ...
            raise DataValidationError(
                f"Failed to parse Yahoo Finance real-time quote: {str(e)}"
            )

    def _parse_intraday_data(self, data: Dict[str, Any]) -> List[OHLCVTData]:
        """Parse intraday data response from Yahoo Finance."""
    ...
    async def get_metrics(self) -> DataSourceMetrics:
        """Get current Yahoo Finance metrics."""
        return self.metrics

    def is_supported_asset_type(self, asset_type: AssetType) -> bool:
        """Check if asset type is supported by Yahoo Finance."""
        # Yahoo Finance supports most asset types
    ...
    def is_supported_interval(self, interval: TimeInterval) -> bool:
        """Check if time interval is supported by Yahoo Finance."""
        # Yahoo Finance supports most intervals
        return interval in [
